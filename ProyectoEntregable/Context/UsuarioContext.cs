...
namespace ProyectoEntregable.Context
{
    public class UsuarioContext
    {
        ...
        public static List<Usuario> ListarProductos()
        {
            List<Usuario> lista = new List<Usuario>();
            string query = UserQuerys.;
                        ...
                        using (SqlDataReader reader = cmd.ExecuteReader())
                        {
                            if (reader.HasRows)
                            {
                                while (reader.Read())
                                {
                                    ...
                                    producto.Descripcion = reader["Description"].ToString();
        ...
        public static void CrearProducto(Producto producto)
        {
            ...
            try
            {
                using (SqlConnection conexion = new SqlConnection(connection))
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand(query, conexion))
                    {
                        cmd.Parameters.Add(new SqlParameter("Descripción", SqlDbType.VarChar) { Value = producto.Descripcion });
